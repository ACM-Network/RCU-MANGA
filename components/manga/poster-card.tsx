...
export const PosterCard = memo(function PosterCard({ manga }: PosterCardProps) {
  ...
  return (
    <Link
      ...
      className="card-hover group block overflow-hidden rounded-[28px] border border-white/10 bg-[#121318]/80 p-3"
    >
      <div className="poster-shadow relative aspect-[3/4] overflow-hidden rounded-[22px]">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/78 via-transparent to-transparent opacity-80 transition group-hover:opacity-95" />
        <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/90 to-transparent" />
        <div className="absolute inset-x-4 bottom-4 flex items-center justify-between rounded-full border border-white/10 bg-black/45 px-3 py-2 text-[11px] uppercase tracking-[0.24em] text-stone-100 backdrop-blur-md">
      ...
      <div className="space-y-3 px-1 pb-1 pt-4">
        <div>
          <h3 className="text-2xl font-semibold text-white">{manga.title}</h3>
